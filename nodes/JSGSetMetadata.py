...
class JSGSetMetadata:
    CATEGORY = "JSG Utils/Metadata"
    FUNCTION = "set"
    RETURN_TYPES = ("JSGMETADATA",)
    RETURN_NAMES = ("Metadata",)
    OUTPUT_TOOLTIPS = ("Returns the metadata object with the specified root-level values set.",)

    DESCRIPTION = (
        "Sets values on a metadata object at the root level."
    )
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "metadata": ("JSGMETADATA", {"tooltip": "The metadata object to modify."}),
                "keys": ("STRING", {"default": "", "multiline": True, "tooltip": "The root-level keys to set, one per line or comma-separated."}),
    ...
    def set(self, metadata, keys, values, value_mode):
        ...
        for i, k in enumerate(key_list):
            out[str(k)] = self._coerce_value(get_value(i), value_mode)
